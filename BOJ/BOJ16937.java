        ...
        for (int i = 0; i < paper.length - 1; i++) {
            ...
            for (int j = i + 1; j < paper.length; j++) {
                ...
                if (x != 0) {
                    ...
                    if ((H >= paper[j][0] && W - paper[i][1] - paper[j][1] >= 0)
                            || (H >= paper[j][1] && W - paper[i][1] - paper[j][0] >= 0)) {
                        max = Math.max(max, x + paper[j][0] * paper[j][1]);
                    }

                    if ((H - paper[i][0] - paper[j][0] >= 0 && W >= paper[j][1])
                            || (H - paper[i][0] - paper[j][1] >= 0 && W >= paper[j][0])) {
                        max = Math.max(max, x + paper[j][0] * paper[j][1]);
                    }
                ...
                if (y != 0) {
                    if ((H >= paper[j][0] && W - paper[i][0] - paper[j][1] >= 0)
                            || (H >= paper[j][1] && W - paper[i][0] - paper[j][0] >= 0)) {
                        max = Math.max(max, y + paper[j][0] * paper[j][1]);
                    }

                    if ((H - paper[i][1] - paper[j][0] >= 0 && W >= paper[j][1])
                            || (H - paper[i][1] - paper[j][1] >= 0 && W >= paper[j][0])) {
                        max = Math.max(max, y + paper[j][0] * paper[j][1]);
                    }
                }
